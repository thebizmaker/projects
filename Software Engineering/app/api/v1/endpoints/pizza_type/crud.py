...
def update_pizza_type(pizza_type: PizzaType, changed_pizza_type: PizzaTypeCreateSchema, db: Session):
    ...
    logging.info(f'Pizza type updated with the name {pizza_type.name}')
    return pizza_type
...
def create_topping_quantity(
    ...
    db.commit()
    db.refresh(pizza_type)
    logging.info(f'Topping quantity for pizza type {pizza_type.name} created with the name {pizza_type.name}')
    return entity
