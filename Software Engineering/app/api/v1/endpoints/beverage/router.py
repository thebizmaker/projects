...
from app.constants import ITEM_NOT_FOUND, BEVERAGE_NOT_FOUND
...
def create_beverage(beverage: BeverageCreateSchema,
    ...
    if beverage_found:
        ...
        logging.warning('Beverage with name {} already exists'.format(beverage_found.name))
...
def update_beverage(
    ...
    if beverage_found:
        if beverage_found.name == changed_beverage.name:
            ...
            logging.info('{} is already the name of the beverage'.format(beverage_found.name))
            return Response(status_code=status.HTTP_204_NO_CONTENT)
        else:
            beverage_name_found = beverage_crud.get_beverage_by_name(changed_beverage.name, db)
            if beverage_name_found:
                logging.warning('Beverage with name {} already exists'.format(beverage_name_found.name))
...
def delete_beverage(
    ...
    logging.info('Beverage with id {} deleted'.format(beverage_id))
    return Response(status_code=status.HTTP_204_NO_CONTENT)
