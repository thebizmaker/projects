...
def create_address(schema: AddressCreateSchema, db: Session):
    ...
    db.add(entity)
    db.commit()
    logging.info('Created new address with id {}'.format(entity.id))
    return entity
...
def delete_address_by_id(address_id: uuid.UUID, db: Session):
    ...
    if entity:
        db.delete(entity)
        db.commit()
        logging.info('Deleted address with id {}'.format(entity.id))
...
def update_address(address: Address, changed_address: AddressCreateSchema, db: Session):
    ...
    db.commit()
    db.refresh(address)
    logging.info('Updated address with id {}'.format(address.id))
    return address
