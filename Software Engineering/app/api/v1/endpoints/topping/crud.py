...
def create_topping(schema: ToppingCreateSchema, db: Session):
    ...
    db.add(entity)
    db.commit()
    logging.info('Topping with id {} created'.format(entity.id))
    return entity
...
def update_topping(topping: Topping, changed_topping: ToppingCreateSchema, db: Session):
    ...
    db.commit()
    db.refresh(topping)
    logging.info('Topping with id {} updated'.format(topping.id))
    return topping
...
def delete_topping_by_id(topping_id: uuid.UUID, db: Session):
    ...
    if entity:
        db.delete(entity)
        db.commit()
        logging.info('Topping with id {} deleted'.format(entity.id))
