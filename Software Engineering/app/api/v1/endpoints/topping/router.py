...
from app.constants import ITEM_NOT_FOUND, TOPPING_NOT_FOUND
...
def create_topping(topping: ToppingCreateSchema,
    ...
    if topping_found:
        ...
        logging.warning('Topping with id {} already exists'.format(topping_found.id))
        return RedirectResponse(url=url, status_code=status.HTTP_303_SEE_OTHER)

    new_topping = topping_crud.create_topping(topping, db)
    logging.info('Topping with id {} created.'.format(new_topping.id))
    return new_topping
...
def update_topping(
    ...
    if topping_found:
        if topping_found.name == changed_topping.name:
            ...
            logging.info('Topping with id {} updated.'.format(topping_found.id))
            ...
            if topping_name_found:
                ...
                logging.warning('Topping with name {} already exists.'.format(topping_name_found.name))
                return RedirectResponse(url=url, status_code=status.HTTP_303_SEE_OTHER)
            else:
                updated_topping = topping_crud.create_topping(changed_topping, db)
                logging.info('Topping with name {} updated.'.format(updated_topping.name))
                response.status_code = status.HTTP_201_CREATED
    else:
        logging.error(TOPPING_NOT_FOUND.format(topping_id))
